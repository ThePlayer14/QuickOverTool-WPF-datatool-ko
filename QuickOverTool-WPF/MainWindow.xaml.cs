    ...
    public partial class MainWindow : Window
    {
        ...
        public void FlushInformation()
        {
            ...
            else
            {
                ...
                labelOverToolExecutable.Content = "찾을 수 없음";
            }
            // Ow.keys
            if (dataTool[1] != null)
            {
                labelOverToolIntegrity.Foreground = new SolidColorBrush(Colors.Green);
                labelOverToolIntegrity.Content = "찾았다";
            }
            else
            {
                labelOverToolIntegrity.Foreground = new SolidColorBrush(Colors.Red);
                labelOverToolIntegrity.Content = "찾을 수 없음";
            }
            // Overwatch
            if (overwatch != null)
            {
                labelValidity.Content = "Overwatch 가 유효함";
                ...
                if (overwatch[1] == "PTR")
                {
                    AddLog("(전위) PTR 빌드가 감지됨 DataTool은 PTR과 호환되지 않는다.");
                    labelOverwatchBranch.Foreground = new SolidColorBrush(Colors.Red);
                } 
            }
            else
            {
                labelValidity.Content = "Overwatch 가 잘못됨";
                ...
                labelOverwatchVersion.Content = "사용할 수 없음";
                labelOverwatchBranch.Foreground = new SolidColorBrush(Colors.Red);
                labelOverwatchBranch.Content = "사용할 수 없음";
            }
        }
        // Log increment
        ...
        private void buttonOutputPath_Click(object sender, RoutedEventArgs e)
        {
            ...
            AddLog("출력 경로: " + textBoxOutputPath.Text);
        ...
        private void buttonStart_Click(object sender, RoutedEventArgs e)
        {
            ...
            // Launch
            AddLog("지금 시간: " + DateTime.Now.ToString());
            AddLog("Cmdline: DataTool.exe" + command);
            AddLog("출력: " + textBoxOutputPath.Text);
            StartUp(command);
        }
        ...
        {
            AddLog(DateTime.Now.ToString() + " - DataTool이 종료됨.");
        }
        ...
        private void buttonSaveOutput_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                textBoxOutput.Text = "저장된 로그 대상 " + Path.Combine(textBoxOutputPath.Text, "log.txt");
            }
            catch
            {
                textBoxOutputPath.BorderBrush = new SolidColorBrush(Colors.Red);
                AddLog("출력 경로가 잘못되었거나 사용 권한이 거부됨.");
            }
        }
        ...
        private void buttonSaveCmdline_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                AddLog("모드에 대해 작성된 배치 파일 " + GetRadioButton() +
                    " to _" + GetRadioButton() + ".bat.");
            }
            catch
            {
                AddLog("설정이 잘못됨. 모드 설정을 확인하십시오.");
            }
        }
        ...
        private void buttonTaskkill_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                DialogResult prompt = System.Windows.Forms.MessageBox.
                    Show("DataTool이 아직 실행 중임. 종료하시겠습니까?", "확인", MessageBoxButtons.YesNo);
                if (prompt == System.Windows.Forms.DialogResult.Yes)
                {
                    proc.Kill();
                    AddLog("DataTool이 종료됨.");
                }
                else if (prompt == System.Windows.Forms.DialogResult.No) return;
            }
            catch
            {
                AddLog("종료 실패. DataTool이 실행되고 있지 않을 수 있음.");
                return;
            }
        }
